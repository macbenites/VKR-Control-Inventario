...
<sql:query var="sqlusuario" dataSource="jdbc/calprusof">
    select T1.id_usuario,T2.id_empleado,T2.nom_emp,T1.nom_usu,T1.con_usu,T1.tipo_usuario
    from usuario T1
    inner join empleado T2 on T2.id_empleado=T1.id_emp
    ORDER BY T1.id_usuario ASC;
</sql:query>
...
<html lang="en">
    <head>
        ...
        <title>VKR | Lista de Usuarios</title>
        ...
        <script>
            function Editar(cod, nom, usu, con, tusu) {
                document.getElementById("titulomodal").innerHTML = "Usuario: " + usu;
                document.getElementById("textoGuardar").innerHTML = "Actualizar Usuario";
                ...
                document.getElementById("txtContraseña").value = con;
                document.getElementById("selUsu").value = tusu;
            ...
            function Eliminar(cod, nom, usu, con, tusu) {
                document.getElementById("titulomodal").innerHTML = "Eliminar Usuario: " + nom;
                document.getElementById("textoGuardar").innerHTML = "Eliminar Usuario";
                document.getElementById("txtCodigo").value = cod;
                document.getElementById("txtNombre").value = nom;
                document.getElementById("txtUsuario").value = usu;
                document.getElementById("txtContraseña").value = con;
            ...
            function Guardaru() {
                ...
                tip = document.getElementById("selUsu").value;
                op = document.getElementById("opc").value;
                datos = {codigo: cod, usuario: usu, contraseña: con, tipo: tip, opc: op};
                $.ajax({
                    data: datos,
                    type: 'POST',
                    url: "MantenimientovUsu.jsp",
            ...
            <div class="content-wrapper">
                <div class="content">
                    ...
                    <table id="datos" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>Codigo</th><th>Nombre</th><th>Usuario</th><th>Tipo de Usuario</th><th colspan="2"><input size=10 type="text" name="table_search" id="searchTerm" onkeyup="doSearch()" class="form-control float-right" placeholder="Buscar"></th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach var="i" items="${sqlusuario.rows}">
                                <tr>
                                    <td>${i.id_usuario}</td>
                                    <td>${i.nom_emp}</td>
                                    <td>${i.nom_usu}</td>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-block btn-info btn-sm" onclick="Editar(${i.id_usuario}, '${i.nom_emp}', '${i.nom_usu}', '${i.con_usu}', '${i.tipo_usuario}')">Editar</button>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-block btn-warning btn-sm" onclick="Eliminar(${i.id_usuario}, '${i.nom_emp}', '${i.nom_usu}', '${i.con_usu}', '${i.tipo_usuario}')">Eliminar</button>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input readonly=»readonly» type="text" class="form-control" id="txtNombre">
                                    </div>
                                    <div class="form-group">
                                        <label for="txtUsuario">Usuario</label>
                                        <input type="text" class="form-control obligatorio" id="txtUsuario">
                                    </div>  
                                    <div class="form-group">
                                        <label for="txtContraseña">Contraseña</label>
                                        <input type="password" class="form-control obligatorio" id="txtContraseña" onkeyup="this.form.txtContraseña1.value = md5(this.form.txtContraseña.value)">
                                        <input type="hidden" id="txtContraseña1">
                                    </div>
                                    <div class="form-group">
                                        <label for="txtTipoUsuario">Tipo de Usuario</label>
                                        <select class="form-control select2" id="selUsu" style="width: 100%;" <jsp:include page="LiTipoUsuario.jsp"/></select>
                                    </div>
                                    <label>* Deberá escribir la contraseña nuevamente.</label>
